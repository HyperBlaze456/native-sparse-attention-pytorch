...
class SparseAttention(Module):
    ...
    def __init__(
        ...
        # Separate GQA and MHA: if kv_heads < heads, we are in GQA mode.
        if kv_heads < heads:
            self.mode = 'GQA'
    ...
    def forward(
        ...
        # QKV projection.
        qkv = self.to_qkv(inp)  # shape: (b, seq_len, dim_inner + 2*dim_kv_inner)
        q, k, v = qkv.split(self.qkv_split, dim=-1)
        q, k, v = map(self.split_heads,
                      (q, k, v))  # shapes: (b, heads, seq_len, d) or (b, kv_heads, seq_len, d) for k, v
