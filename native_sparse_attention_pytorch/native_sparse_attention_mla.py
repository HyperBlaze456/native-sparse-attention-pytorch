...
def create_fine_mask(seq_len, fine_block_size):
    def inner(selected_block_indices: Tensor, num_grouped_queries=1):
        ...
        one_hot_selected_block_indices = torch.zeros((*selected_block_indices.shape[:-1], seq_len // fine_block_size),
        ...
        block_mask = create_block_mask(fine_mask, B=batch, H=kv_heads * num_grouped_queries, Q_LEN=seq_len,
...
class SparseAttention(Module):
    def __init__(
        ...
        self.qk_NoPE_dim = dim_head // 2 # Now we use this. This will replace run_k.
        ...
        # -------------------------------------------------------------
        # NSA Branches: Sliding Window, Block-wise compression, and Fine grained attention based off selection.

        # Local sliding window strategy
    ...
    def forward_inference(
        ...
        q, kv = self.to_qkv(inp).split(self.q_kv_split, dim=-1)

        q, kv = map(self.split_heads, (q, k, v))
        ...
        if has_selected_kv_for_fine_attn:
            ...
            sel_indices = repeat(sel_indices, 'b h 1 sel -> b h sel j d', j=self.selection_block_size,
                                 d=sel_fk.shape[-1])
        ...
        out = einsum(strategy_weighted_combine, stack([compressed_attn_out, fine_attn_out, sliding_window_attn_out]),
    ...
    def forward(
        ...
        if has_selected_kv_for_fine_attn:
            ...
            if self.use_triton_kernel and not disable_triton_kernel:
                ...
                selected_block_indices = cat((selected_block_indices, fine_window_seq),
                                             dim=-1)  # for the block causal diagonal in fig2
        ...
        if exists(sliding_window_flex_mask):
            ...
            sk, sv = tuple(
                repeat(t, 'b h ... -> b (h num_grouped_queries) ...', num_grouped_queries=self.num_grouped_queries) for
                t in (sk, sv))
